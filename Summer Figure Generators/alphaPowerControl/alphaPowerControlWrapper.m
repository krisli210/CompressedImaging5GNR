...
%     prm.RangeBins = prm.rMin : prm.delta_R : prm.rMax;
    ...
    u_range = 1:3:7;
    v_range = 8 - u_range;
    psnrs = zeros(length(v_range), length(alpha_range), nIters);
    sum_capacities = zeros(length(v_range), length(alpha_range), nIters);
    ...
    for alpha_ind = 1:length(alpha_range)
        ...
        for v_ind = 1:length(v_range)
            ...
            for n = 1:nIters
                [sum_capacities(v_ind, alpha_ind, n), psnrs(v_ind, alpha_ind, n)] = alphaPowerControlWrapped(prm, H_TX, H_RX, Psi_AZ, Psi_R);
            end
        end
    end

    save("C:\Users\krisl\Desktop\Summer2023\CompressedImaging5GNR\Summer Figure Generators\alphaPowerControl\alphaPowerControl_vary_V_uniform_Scats_fix_U_prime_extra_Iters", ...
